const Pagination = ({
  ...
  return (
    <div className='flex justify-center items-center py-4 gap-1'>
        ...
        number === 1 || number === totalPages || Math.abs(number - currentPage) <= 1 ? (
          <button
            ...
            className={`px-2 py-1 rounded ${
              number === currentPage ? 'bg-white text-countTextBg shadow-card shadow-black font-bold' : 'bg-white text-gray-500'
            }`}
          >
            {number}
          </button>
      ...
      <button onClick={() => handleClick(currentPage + 1)} disabled={currentPage === totalPages} className='px-2 py-1'>
        {'>'}
      </button>
    </div>
  );
